...
var setting = {
    async: {
        ...
        dataType: "json",
        type: "post",
        url: "http://192.168.0.239:8080/FaceManage/department/queryAll", //请求节点生成的servlet
        autoParam: ["id"]
        //每次异步加载传给服务器的参数
    },
    view: {
...
function onClick(event, treeId, treeNode) {
    ...
    if (treeNode.isParent && treeNode.id != 1) {
        url = "http://192.168.0.239:8080/FaceManage/department/listAll?dId=" + treeNode.id;
    } else if (treeNode.isParent && treeNode.id == 1) {
        url = "http://192.168.0.239:8080/FaceManage/department/listAll";
    } else {
        url = "http://192.168.0.239:8080/FaceManage/department/listAll?cId=" + treeNode.id;
    }
...
function beforeRemove(treeId, treeNode) {
    if (treeNode.pId == 0) { //如果删除的是根节点，也提示无法删除
        ...
        $("#deptModel #continue").click(function() {
            $.ajax({
                type: 'POST',
                url: 'http://192.168.0.239:8080/FaceManage/department/delete?',
                data:{
                  dId:treeNode.id
                },
                success: function() {
...
function beforeRename(treeId, treeNode, newName, isCancel) {
    ...
    if (newName == "") { // 新名称为空的情况
        ...
        $.ajax({
            type: 'post',
            url: "http://192.168.0.239:8080/FaceManage/department/editDepartment", //更改请求
            data: {
                did: treeNode.id,
                name: newName,
                dpId: treeNode.pId
    ...
    if (btn)
        btn.bind("click", function() {
            ...
            $.ajax({ //后端添加！！！！   必须有，要不数据库还是没添加，否则刷新页面后节点就会消失！
                    url: "http://192.168.0.239:8080/FaceManage/department/addDepartment",
...
$.ajax({
    type: 'POST',
    url: 'http://192.168.0.239:8080/FaceManage/department/initDeptList',
    success: function(data) {
...
function deleteDept(row) {     //点击删除按钮方法
    ...
    $("#deptModel #continue").click(function() {
        $.ajax({
            type: 'POST',
            url: 'http://192.168.0.239:8080/FaceManage/department/delete?dId=' + row.did,
            success: function() {
                $("#modal-body-text").text("删除成功!");
                $("#myModalLabel").modal();
                LoadAjaxContent('http://192.168.0.239:8080/FaceManage/department/initDeptList');
...
function deptEdit(row) {
  // row.dDidconsole.log(row.dDid)
    ...
    $("#deptInfoModal .modal-title").html("修改");
    $("#deptInfoModal").modal();
    $("#deptInfoModal #cancel").click(function() {
        $("#deptTable").bootstrapTable('refresh');
        $("#departmentForm")[0].reset();
    });

    $("#deptInfoModal #continue").click(function() {    //添加按钮
        var name = $("#name").val();
        var dpId = $("#dPId").val();
        var did = row.did;
        $.ajax({
            ...
            success: function(data) {
              console.log(data)
                if (data.code == 200) {
                    ...
                    $("#modal-body-text").html("添加成功");
                    $('#deptModel').modal('show');
                    LoadAjaxContent('http://192.168.0.239:8080/FaceManage/department/selectAll');
                } else {
                    $("#myModalLabelLabel").html("提示");
                    $("#modal-body-text").html(data.msg);
                    $('#deptModel').modal('show');
