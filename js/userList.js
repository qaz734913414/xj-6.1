...
function init() {
    ...
    $.ajax({
        type: 'POST',
        url: 'http://192.168.0.239:8080/FaceManage/user/initDeptAndRole',
        success: function(data) {
            ...
            for (var i = 0; i < rListData.length; i++) {
                  roleStr += '<option value="' + rListData[i].rId + '">' + rListData[i].rName + '</option>';
              }
            // function(data){
            //   for (var i = 0; i < dListData.length; i++) {
            //       unitStr += '<option value="' + data[i].did + '">' + data[i].dname + '</option>';
            //   }
            // }
...
function getTable() {
    ...
    $("#userTable").bootstrapTable({
        method: "post",
        url: "http://192.168.0.239:8080/FaceManage/user/usersList",
        queryParams: {
...
function updateStatus(row) {
  console.log(row.uId)
    $.ajax({
        type: 'POST',
        url: 'http://192.168.0.239:8080/FaceManage/user/status',
        data:{
          uId:row.uId
        },
        success: function() {
...
function toRemove() {
    ...
    if (ids) {
        ...
        $("#deptModel #continue").click(function() {
            ...
            $.ajax({
                type: 'POST',
                url: "http://192.168.0.239:8080/FaceManage/user/deleteByIds",
                data:{
                  ids:ids
                },
                success: function() {
...
function deleteUser(row) {
  console.log(row)
    ...
    $("#deptModel #continue").click(function() {
        ...
        $.ajax({
            type: 'POST',
            url: 'http://192.168.0.239:8080/FaceManage/user/delete',
            data:{
              id:row.uId
            },
            success: function() {
...
function resetPwd(row) {
    ...
    $("#deptModel #continue").click(function() {
        ...
        $.ajax({
            type: 'POST',
            url: 'http://192.168.0.239:8080/FaceManage/user/resetpwd',
            data: {
                uId: row.uId
            },
            success: function() {
...
function userEdit(row) {
    ...
    $("#userModal #continue").click(function() {
        ...
        $.ajax({
            type: 'post',
            url: 'http://192.168.0.239:8080/FaceManage/user/edit',
            data: {
                uId: uId,
                uName: $("#uName").val(),
                uRealName: $("#urealName").val(),
              ...
              console.log(data)
                if (data.code == 0) {
                  console.log('修改成功')
                    ...
                    $('#myModal').modal('show');
                    LoadAjaxContent('http://192.168.0.239:8080/FaceManage/user/list');
                } else {
                    $("#myModalLabel").html("提示");
                    $("#modal-body-id").html(data.msg);
                    $('#myModal').modal('show');
...
function addUser() {
    ...
    $("#userModal #continue").click(function(form) {
        ...
        $.ajax({
            type: 'post',
            url: 'http://192.168.0.239:8080/FaceManage/user/new',
            data: {
                uName: $("#uName").val(),
                uRealName: $("#urealName").val(),
                uSex: $("#userModal input[name='uSex']:checked").val(),
...
function doUpload() {
    ...
    $("#fileinputModal #continue").click(function() {
        ...
        console.log(formData);
        $.ajax({
            url: 'http://192.168.0.239:8080/FaceManage/fileUpload',
            type: 'POST',
            data: data,
            async: false,
            cache: false,
            ...
            success: function(data) {
                if (data.code == 0) {
                    ...
                    $("#modal-body-id").text("上传失败,请上传正确格式的表格!");
                    $("#myModal").modal();
                }
            },
            error: function(data) {
